...
def test_range_uniqueness():
    for n in range(1, 5):
        ...
        # queries
        history = set()
        for index in range(min([_range - 1, 256 ** 2])):
...
async def test_sizes():
    for n in range(SequenceID._MIN_SIZE, SequenceID._MAX_SIZE):
        ...
        context = "The salt was allowed to be less than the length of the declared size of sequence ids"
        with ignore(ValueError, if_else=violation(context)):
            sid = SequenceID(salt=token_bytes(n - 1), size=n)

        sid = SequenceID(token_bytes(2 * n), size=n)
