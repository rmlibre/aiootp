...
A: For sure, one-time pads are stream ciphers. Though, if we trust that pseudo-random functions **(PRFs)** exist, then by definition, their outputs are indistinguishable from truly random bits. Because of this, it's proven that pseudo_ one-time pads are computationally secure if they use secure PRFs. We conjecture that the sha3_512 hash functions are either PRFs, or are close with negligible difference. In our view, they're ideal candidates for the role of mimicking PRFs because: 
 ...
 -  when updated with new key material, the state is xor'd with the key materal then permuted with the hidden state, which is essentially one-time-pad encryption of the internal state with additional mixing
